...
- [ ] Write failing tests in `src/project/adoption-plan.test.ts`, following R1's existing plan-builder test fixtures (tmp project + injected state). Add:
  ...
  describe('map-candidate disposition (R3, D10)', () => {
    ...
    it('first-touch (no .scaffold/, state=null) annotates run rows adoption from plan.mode', () => {
      // fixture: brownfield tmp project with NO .scaffold/ directory — the primary
      // D2 entry path where buildAdoptionPlan's state is null. Must not crash and
      // must annotate uncompleted run rows as 'adoption' (plan.mode==='brownfield').
      const plan = buildAdoptionPlan(/* brownfield fixture, NO .scaffold/state.json */)
      const row = plan.steps.find(s => s.step === 'tech-stack')
      expect(row?.disposition).toBe('run')
      expect(row?.mode).toBe('adoption')
    })
