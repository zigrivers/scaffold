...
Domain 08 (Prompt Frontmatter) defines the `reads` field in prompt frontmatter, which allows a prompt to declare which predecessor steps it reads from. Domain 11 (Decision Log) defines `decisions.jsonl`, which captures key decisions made during prompt execution.
...
1. **Direct artifact reads via the `reads` field**: The prompt frontmatter's `reads` field declares which predecessor steps the prompt reads from, using step names. During execution, the scaffold CLI resolves each step name to its output artifact and provides that content to the LLM as context. For example, `api-design` declares `reads: [define-tech-stack]` to receive the tech-stack output artifact. (Dependency-chain artifacts are included automatically; `reads` adds cross-cutting references. See ADR-050 and ADR-053.)
...
- Step-name references in the `reads` field depend on stable step names in the pipeline — if a step is renamed, downstream `reads` declarations must be updated
...
- Cross-prompt data sharing MUST use the `reads` field in prompt frontmatter for artifact content and `decisions.jsonl` for structured decision data
- The `reads` field MUST accept step names (not file paths) and resolve them via the step registry (per ADR-050 and ADR-053)
...
- [ADR-015](ADR-015-prompt-frontmatter-schema.md) — Frontmatter schema includes the `reads` field for declaring predecessor step dependencies
- [ADR-050](ADR-050-context-window-management.md) — Context window management; establishes the `reads` field with step-name syntax
- [ADR-053](ADR-053-artifact-context-scope.md) — Artifact context scope; specifies dependency-chain defaults and `reads` extensions
- [ADR-034](ADR-034-rerun-no-cascade.md) — Re-runs do not cascade; context.json would have complicated re-run semantics
- Domain 08 ([08-prompt-frontmatter.md](../domain-models/08-prompt-frontmatter.md)) — Frontmatter schema including the `reads` field and step-name resolution
