...
| 7.4 | MCP tool references are Claude Code-only | Medium | `add-playwright.md`, `claude-code-permissions.md` | Prompts reference MCP (Model Context Protocol) tools and Playwright MCP configuration. Codex has no MCP concept. The adapter should either strip MCP sections for Codex or replace them with equivalent Codex-native instructions. |
