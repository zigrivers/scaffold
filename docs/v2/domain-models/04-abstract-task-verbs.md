...
| `sync` | `if remote=$(bd config get dolt.remote 2>/dev/null) && [ -n "$remote" ]; then bd dolt push; fi` (exit-0 no-op when no Dolt remote is configured; safe under `set -e`) | inline-code |
...
close:
  ...
  format: prose
sync:
  template: '`if remote=$(bd config get dolt.remote 2>/dev/null) && [ -n "$remote" ]; then bd dolt push; fi` (exit-0 no-op when no Dolt remote is configured)'
  format: prose
```
...
6. Close the tracking task:
   `bd close TASK_ID`
   `if remote=$(bd config get dolt.remote 2>/dev/null) && [ -n "$remote" ]; then bd dolt push; fi`  # exit-0 no-op when no Dolt remote is configured
```
