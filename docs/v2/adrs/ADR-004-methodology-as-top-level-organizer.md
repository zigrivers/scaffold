...
1. **Methodology-first**: Users choose a methodology (e.g., "classic", "classic-lite") that defines the pipeline shape — phases, prompt ordering, dependencies, and axis defaults. Axes are customization points within the methodology.
...
Concretely:
- `.scaffold/config.yml` specifies exactly one methodology (e.g., `methodology: classic`)
...
- Users customize by selecting mixin values for the methodology's declared axes (e.g., `task-tracking: github-issues` within the `classic` methodology)
...
- **Methodologies are coherent philosophies, not mix-and-match parts**: A methodology represents an opinionated workflow — "classic" includes 7 phases, task tracking via Beads, multi-agent parallel execution, and comprehensive documentation. "classic-lite" might have 4 phases and skip multi-agent setup. These are fundamentally different workflows, not just different values on the same axes. Mixing phases from "classic" with the simplified flow of "classic-lite" produces an incoherent pipeline with missing dependencies and orphaned prompts.

- **Different methodologies have fundamentally different pipeline shapes**: Classic has phases 0-7 with specific prompt ordering and dependency constraints documented in the Setup Order table. A hypothetical "rapid" methodology might compress this into 3 phases with different prompts entirely. The phase structure is not a parameter — it is a defining characteristic of the methodology. Domain 01, Section 3 (Resolution Rules) shows how the methodology manifest controls prompt ordering and dependency resolution.
...
### Negative
- Less flexibility than axis-first — users cannot freely mix phases or prompts across methodologies. A user who wants "classic phases 0-3 then rapid phases 4-5" must create a custom methodology
...
- The number of built-in methodologies affects user experience — too few and users feel constrained, too many and selection is overwhelming. Initial v2 ships with 2-3 methodologies (classic, classic-lite, and potentially a rapid/minimal option)
