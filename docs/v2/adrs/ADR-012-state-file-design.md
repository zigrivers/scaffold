...
- On re-run (`scaffold run --from`), old completion data for the targeted prompt is overwritten, not preserved alongside. The prompt's state entry is reset to `pending` and its previous completion metadata (timestamp, actor) is discarded.
