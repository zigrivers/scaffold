...
| `mmr critique [input] [--focus …] [--context repo] [--no-synthesis] [--format text\|json]` | Multi-model **design/brainstorm critique** of an artifact (a design doc, a pasted "problem + proposed solution", or a plan). Reports **convergence** (where models agreed), **divergence** (genuine splits + the deciding crux), and an editorial **synthesis** that never picks a winner. **Advisory: no pass/fail gate, always exits 0.** A peer to `review`, not a code review. `--context repo` (or `--context-paths a.ts …`) grounds the critique in the codebase; `--no-synthesis` skips the synthesis pass. |
