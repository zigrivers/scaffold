...
| `mmr critique [input] [--focus …] [--format text\|json]` | Multi-model **design/brainstorm critique** of an artifact (a design doc, a pasted "problem + proposed solution", or a plan). Fans it out to the channels and reports where independent models converge vs. diverge. **Advisory: no pass/fail gate, always exits 0.** A peer to `review`, not a code review. |
