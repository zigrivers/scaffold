...
5. **PR title** follows commit message format with the work-item ID leading: `<id>: type(scope): description` — under squash-merge the title becomes the commit subject on main, so the leading ID is what makes `git log --oneline` traceable per task; the PR body still carries the canonical machine-readable reference (`Closes <id>` / `Refs T-XXX`)
6. **PR body** includes:
