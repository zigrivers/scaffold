...
Applications that fail to handle errors and exceptional conditions securely, leading to information disclosure, denial of service, or other security breaches.

**Examples:**
- Stack traces exposed to end users revealing internal paths and logic
- Generic catch-all error handlers that mask security-relevant failures
- Inconsistent error responses that allow user enumeration (e.g., different messages for "user not found" vs. "wrong password")
- Failing to fail securely — leaving systems in an inconsistent or permissive state after an error

**Mitigations:**
- Implement centralized, consistent error handling that returns safe, generic messages to users
- Log detailed error information server-side for debugging and monitoring
