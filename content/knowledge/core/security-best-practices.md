...
- Error messages that reveal system internals (database schema, file paths)
- Fail-open security controls (access granted when authorization service is down)
- Inconsistent error handling leading to privilege escalation

**Mitigations:**
- Use generic error messages for all user-facing errors
- Log detailed error information server-side only
- Implement fail-secure defaults (deny access when authorization cannot be verified)
