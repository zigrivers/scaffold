...
async function configInit(opts: { withExamples: boolean } = { withExamples: false }): Promise<void> {
  ...
  const template = [
    ...
    '  fix_threshold: P2',
    '  # min_completed_channels: how many channels must actually report before',
    '  # a passing gate counts as a pass. Below it the verdict is',
    '  # needs-user-decision (exit 3) — one channel agreeing with itself is not',
    '  # corroboration. Set to 1 to accept single-channel approval.',
