...
3. **Pipeline files live in phase subdirectories**: `pipeline/{phase}/{step}.md`.
   Always search all subdirectories (`pipeline/**/*.md`), never guess paths.

4. **Tool-scoped knowledge entries** in `knowledge/tools/` are referenced by
   commands only, not pipeline steps. Check `commands/` before flagging as unused.
...
Read the most recent audit report under `docs/comprehensive-alignment-audit-round-*.md`
...
Read ALL pipeline step files in `pipeline/**/*.md`. For each step:
...
preset files in `methodology/*.yml`.
...
Read ALL pipeline step Quality Criteria sections in `pipeline/**/*.md`.
...
Read ALL knowledge entry frontmatter in `knowledge/**/*.md` AND all
...
2. **Unused entries**: Are there knowledge entries that no pipeline step
   references? Check `commands/` before flagging — tool-scoped entries
   are expected to be command-only.
...
5. **Knowledge ↔ step quality**: For 10 representative steps
   ...
   entries actually help Claude produce better output. Flag entries
...
**Question**: Have commands drifted from their pipeline sources since
the last `scaffold build`?

Read 15 representative command files and their pipeline counterparts:
create-prd, tdd, story-tests, create-evals, system-architecture,
domain-modeling, implementation-plan, traceability-matrix,
apply-fixes-and-freeze, implementation-playbook, review-architecture,
security, operations, coding-standards, database-schema.

1. **Content drift**: Compare each command's process instructions
   against the pipeline step's body. Flag meaningful divergences (not
   cosmetic differences like whitespace or word order).

2. **Mode Detection parity**: Does the command's Mode Detection check
   the same artifact as the pipeline step's?

3. **After This Step accuracy**: Do the command's next-step
   recommendations match the pipeline's dependency graph?

4. **Depth handling**: Commands are pre-rendered. Do they handle
   methodology depth correctly, or do they always produce maximum-depth
   output regardless of the user's methodology setting?

5. **Staleness signal**: For each pair, check if the pipeline step's
   modification date is newer than the command's. If so, the command
   may be stale. (Use frontmatter or tracking comments if available;
   fall back to git log.)

Deliverable: Parity assessment with specific divergences.
...
6. **Post-pipeline workflows**: Do the ongoing commands
...
Produce `docs/comprehensive-alignment-audit-round-N.md` (increment N
...
| 5. Command Parity (Build Drift) | Commands diverging from pipeline, stale commands |
