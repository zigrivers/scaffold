...
export const gateTargetsCheck: DoctorCheck = {
  ...
  run: (ctx) => {
    ...
    // never "healthy" — unless a generated gate script exists, in which case
    // D5 (R2) upgrades this to a bounded GATE_PROBE=1 execution probe below.
    const probe = runGateProbe(ctx.projectRoot)
    if (probe.ran) {
      if (probe.ok) {
        // Upgrade: resolution + a PASSING bounded probe is a real health signal.
        return res(gateTargetsCheck, 'ok', probe.detail)
      }
      // A failing probe is a gate-section failure — same tier as the
      // targets-missing case above ('warn'), with the probe's own detail.
