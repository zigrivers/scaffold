...
function ledgerTaskClaimed(taskId: string, ts: string): Event {
  return {
    event_id: `ulid-tc-${ts}`, worktree_id: 'wid-1', actor_label: 'agent-alice',
...
function ledgerPrOpened(prNum: number, ts: string): Event {
  return {
    event_id: `ulid-pr-${ts}`, worktree_id: 'wid-1', actor_label: 'agent-alice',
    branch: 'feat', task_id: null, type: 'pr_opened', ts,
    payload: { pr_number: prNum, url: '' },
  } as Event
}
...
function prMergedReplayEvent(prNum: number, ts: string): ReplayEvent {
  return {
    sort_id: `gh:${prNum}:merged`, correlation_id: `pr:${prNum}:merged`, ts,
    source: 'gh', kind: 'pr_merged', summary: `PR #${prNum} merged`,
  }
}
...
describe('evaluateStall', () => {
  it('emits task_stale when task_claimed has no commits or completion past threshold', () => {
    ...
    const items = evaluateStall({
      now: NOW, ledgerEvents: events, replayEvents: [], findings: [],
  ...
  it('emits audit_findings_unresolved when findings above threshold are open past 24h', () => {
    ...
    const items = evaluateStall({
      now: NOW, ledgerEvents: [], replayEvents: [], findings: [oldFinding],
      config: DEFAULT_CONFIG, lensSkippedStreaks: {},
    })
  ...
  it('emits lens_skipped_repeatedly when a lens streak is ≥ 3', () => {
    const items = evaluateStall({
      now: NOW, ledgerEvents: [], replayEvents: [], findings: [],
      config: DEFAULT_CONFIG, lensSkippedStreaks: { 'B-ac-coverage': 4 },
    })
  ...
  it('does not emit a signal when its threshold is configured "off"', () => {
    ...
    const items = evaluateStall({
      now: NOW, ledgerEvents: events, replayEvents: [], findings: [], config, lensSkippedStreaks: {},
    })
    expect(items.find((i) => i.signal === 'task_stale')).toBeUndefined()
  })
})
