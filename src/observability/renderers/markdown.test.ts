...
const fixture: EngineOutput = {
  ...
  snapshot: {
    ...
    completed_in_window: [{
      task_id: 'T-029', task_title: 'login bug', outcome: 'pr_submitted', pr_number: 40, by: 'agent-alice',
    }],
    in_flight: [{
      task_id: 'T-031', task_title: 'refresh token rotation', by: 'agent-alice',
      claimed_at: '2026-05-04T13:55:00Z', age_hours: 0.1, branch: 'feat-auth',
    }],
    blocked: [], upcoming: [],
    recent_decisions: [{
      decision_id: 'decision:foo', key: 'foo', summary: 'bar',
      recorded_at: '2026-05-04T13:00:00Z', affects: ['src/foo/**'],
    }],
    story_coverage: [],
  },
...
describe('renderProgressMarkdown', () => {
  ...
  it('omits empty sections', () => {
    const empty = {
      ...fixture,
      snapshot: {
        ...fixture.snapshot!, active_agents: [], in_flight: [], completed_in_window: [], recent_decisions: [],
      },
    }
  ...
  const auditFixture: EngineOutput = {
    ...
    findings: [
      ...
      { id: '9d1e02f455667788', lens_id: 'A-tdd', severity: 'P1',
        ...
        confidence: 'medium', first_seen: '2026-05-04T00:00:00Z', last_seen: '2026-05-04T00:00:00Z',
        status: 'acknowledged', ack_note: 'tracked separately' },
    ],
