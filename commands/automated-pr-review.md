---
description: "Set up automated PR review with external AI reviewers"
long-description: "Configures agent-driven automated PR review using external AI reviewers (Codex Cloud, Gemini Code Assist, or custom). The agent manages the entire review-fix-push loop locally — zero GitHub Actions workflows, zero Actions minutes consumed."
---

Set up automated PR review for this project. The system uses an external AI reviewer to catch issues that local self-review misses, with the agent managing the entire fix loop locally.

**This step does NOT create any GitHub Actions workflows.** All review orchestration runs locally via the agent and a polling script that uses `gh api` calls.
...
## Step 1: Choose External Reviewer

Present the user with reviewer options using AskUserQuestionTool:

**Option A — Codex Cloud (default)**
- Requires: ChatGPT subscription (Plus/Pro/Team) + Codex Cloud GitHub App installed
- Bot username: `chatgpt-codex-connector[bot]`
- Reviews PRs via GitHub App integration, posts inline comments
- Cost: ~25 credits per review from ChatGPT subscription

**Option B — Gemini Code Assist**
- Requires: Google Cloud project + Gemini Code Assist enabled on repo
- Bot username: varies by setup (typically `gemini-code-assist[bot]`)
- Reviews PRs via Google Cloud integration
- Cost: included in Google Cloud subscription or free tier

**Option C — Custom reviewer**
- User provides: bot username, approval signal text, review comment format
- Works with any bot that posts GitHub PR reviews

Store the choice for use in AGENTS.md and the await script configuration.
...
|----------|-------------|
| Codex Cloud | ChatGPT subscription + install "ChatGPT Codex Connector" GitHub App on repo |
| Gemini Code Assist | Google Cloud project + enable Gemini Code Assist |
| Custom | Reviewer bot installed and configured on repo |

**Not required** (unlike the previous version):
...
Add or update the "Code Review" section in CLAUDE.md:
...
| Rail | Implementation |
|------|---------------|
| **Round cap** | `--max-rounds` in await script (default 3). Agent merges after cap with follow-up issue. |
| **Timeout** | `--timeout` in await script (default 15 min). Agent merges with timeout note. |
| **Human override** | `/skip-review` or `/lgtm` comment from repo member bypasses review. |
| **Docs-only skip** | Agent checks diff — if only `.md`, `.yaml`, `.json`, `.toml`, `.lock` files changed, skip external review. |
| **Read-only reviewer** | External bot has no write access. Only the agent pushes fixes. |
| **Follow-up tracking** | When round cap hit, agent creates GitHub Issue for tracking unresolved findings. |
...
- Don't hardcode a specific reviewer — always use the configured bot name

## Process

1. Check applicability (GitHub remote, CI setup)
2. Ask user to choose reviewer (Codex Cloud / Gemini / Custom)
3. Verify prerequisites (reviewer app installed)
4. Create docs/review-standards.md
5. Create AGENTS.md with reviewer-specific instructions
6. Create scripts/await-pr-review.sh with configured defaults
7. Update CLAUDE.md with agent-driven review workflow
8. Update docs/git-workflow.md with review integration note
9. If update mode: offer to clean up legacy GitHub Actions workflows
10. Test: create a small test PR and run the await script to verify polling works
