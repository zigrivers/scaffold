---
description: "Analyze session history to find patterns worth automating"
long-description: "Reads Claude Code session data from ~/.claude/ across all projects to identify repeated tasks, workflows, and preferences — then recommends what to build as skills, plugins, agents, and claude.md rules, prioritized by impact and frequency."
argument-hint: "[--project <name>] [--depth shallow|deep] [--output <path>]"
---

Analyze all of your Claude Code sessions on this computer and identify repeated tasks, workflows, decisions, and patterns across every project you've worked on — then recommend what to automate as skills, plugins, agents, and claude.md rules.
...
Examples to detect:
- Always: plan → implement → test → commit
- Always: read file → understand → refactor → verify
- Always: create task → write code → write tests → PR
...
- Transform data between formats (JSON → CSV, screenshot → code, etc.)
...
Ranked by: frequency × time-saved ÷ implementation effort.
...
```
Score = (Impact × 3) + (Frequency × 2) + (Ease × 1)
```
